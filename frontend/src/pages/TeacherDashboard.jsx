...
function TeacherDashboard() {
  ...
  async function loadSets() {
    try {
      setLoadingSets(true);
      const response = await fetch(`${API_BASE_URL}/FlashcardSet`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await response.json();
      if (!response.ok) {
  ...
  async function createSet(e) {
    ...
    try {
      ...
      setSuccessMessage("");
      const response = await fetch(
        `${API_BASE_URL}/FlashcardSet?name=${encodeURIComponent(setName)}&description=${encodeURIComponent(setDescription)}&wordsToPlay=${wordsToPlay}`,
  ...
  async function importFlashcards(e) {
    ...
    try {
      ...
      if (!Array.isArray(flashcards) || flashcards.length === 0) {
        throw new Error("No flashcards were found in the JSON.");
      }
      const formattedFlashcards = flashcards.map((card) => ({
        word: card.word || "",
        meaning: card.meaning || card.correctMeaning || "",
      ...
      if (invalidCard) {
        throw new Error("Every flashcard must have a word and meaning.");
      }
      const response = await fetch(`${API_BASE_URL}/Flashcard/import`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          flashcardSetId: Number(selectedImportSetId),
          flashcards: formattedFlashcards,
        }),
      });
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.message || "Failed to import flashcards.");
      }
      setJsonText("");
      setSuccessMessage(
  ...
  async function assignSet(e) {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.message || "Failed to assign flashcard set.");
      }
      setSelectedStudentId("");
      setSelectedAssignmentSetId("");
      setSuccessMessage("Flashcard set assigned successfully.");
    } catch (error) {
      setError(error.message);
    } finally {
      setAssigning(false);
  ...
  const averageScore =
    students.length > 0
      ? students.reduce(
          (total, student) => total + (student.averageScore || 0),
          0,
        ) / students.length
      : 0;
  return (
      ...
      <main className="teacher-dashboard-content">
        <section className="teacher-welcome">
          ...
          <div>
            ...
            <h1> Hi, {name}! </h1>{" "}
            <p> Manage your students and vocabulary sets. </p>{" "}
          </div>{" "}
          <div className="teacher-welcome-icon"> V </div>
        </section>
        {error && (
        ...
        <section className="teacher-stats">
          ...
          <div className="teacher-stat-card">
            ...
            <div>
              {" "}
              <span>Students</span> <strong>{totalStudents}</strong>{" "}
            </div>{" "}
          </div>
          <div className="teacher-stat-card">
            {" "}
            <div className="teacher-stat-icon blue"> 📚 </div>{" "}
            <div>
              {" "}
              <span>Flashcard sets</span> <strong>{totalSets}</strong>{" "}
            </div>{" "}
          </div>{" "}
          <div className="teacher-stat-card">
            {" "}
            <div className="teacher-stat-icon yellow"> ✦ </div>{" "}
            <div>
              {" "}
              <span>Total words</span> <strong>{totalFlashcards}</strong>{" "}
            </div>{" "}
          </div>{" "}
          <div className="teacher-stat-card">
            {" "}
            <div className="teacher-stat-icon purple"> % </div>{" "}
            <div>
              {" "}
        ...
        <section className="teacher-tools-grid">
          <div className="teacher-tool-card">
            ...
            <div className="teacher-tool-header">
              {" "}
              <div className="teacher-tool-icon green"> + </div>{" "}
              <div>
                {" "}
                <h2> Create a flashcard set</h2>{" "}
                <p> Start a new vocabulary collection. </p>{" "}
              </div>{" "}
            </div>{" "}
            <form onSubmit={createSet}>
              ...
              <div className="teacher-field">
                <label htmlFor="words-to-play">Words to play</label>
                <select
                  id="words-to-play"
                  value={wordsToPlay}
          ...
          <div className="teacher-tool-card">
            ...
            <div className="teacher-tool-header">
              {" "}
              <div className="teacher-tool-icon blue"> ↓ </div>{" "}
              <div>
                {" "}
                <h2> Import flashcards </h2>{" "}
                <p> Add many words at once using JSON. </p>{" "}
              </div>{" "}
            </div>
            <form onSubmit={importFlashcards}>
              ...
              <div className="teacher-field">
                {" "}
                <label htmlFor="import-set"> Flashcard set </label>{" "}
                <select
                  id="import-set"
                  value={selectedImportSetId}
                  onChange={(e) => setSelectedImportSetId(e.target.value)}
                >
                  {" "}
              ...
              <div className="teacher-field">
                ...
                <textarea
                  ...
                  placeholder={`[ {"word": "apple", "meaning": "quả táo", "example": "I ate an apple."}]`}
                  rows="7"
                />{" "}
              </div>{" "}
              <button
        ...
        <section className="teacher-tool-card teacher-assignment-card">
          {" "}
          <div className="teacher-tool-header">
            <div className="teacher-tool-icon yellow"> → </div>{" "}
            <div>
              {" "}
              <h2> Assign a set to a student </h2>{" "}
              <p>
                {" "}
                Choose a student and give them a vocabulary set to
                practice.{" "}
              </p>{" "}
            </div>{" "}
          </div>
          <form className="teacher-assignment-form" onSubmit={assignSet}>
            {" "}
            <div className="teacher-field">
              {" "}
            ...
            <div className="teacher-field">
              {" "}
              <label htmlFor="assignment-set"> Flashcard set </label>{" "}
              <select
                id="assignment-set"
                value={selectedAssignmentSetId}
                onChange={(e) => setSelectedAssignmentSetId(e.target.value)}
              >
                {" "}
                <option value=""> Select a set</option>
                {sets.map((set) => (
        ...
        <section className="teacher-section">
          ...
          <div className="teacher-section-header">
            ...
            <div>
              {" "}
              <p className="teacher-eyebrow"> YOUR STUDENTS </p>{" "}
              <h2> Students </h2>{" "}
            </div>{" "}
            <span>
              {" "}
          ...
          {loadingStudents ? (
            ...
            <div className="teacher-empty">
              ...
              <p>
                {" "}
                Add student accounts in Supabase and assign them to
                yourself.{" "}
              </p>{" "}
            </div>
          ) : (
              ...
              {students.map((student) => (
                ...
                >
                  ...
                  <div className="teacher-student-info">
                    {" "}
                    <h3> {student.name} </h3> <p> @{student.username} </p>{" "}
                  </div>
        ...
        <section className="teacher-section">
          ...
          <div className="teacher-section-header">
            ...
            <div>
              {" "}
              <p className="teacher-eyebrow"> YOUR CONTENT </p>{" "}
          ...
          {loadingSets ? (
            ...
            <div className="teacher-empty">
              ...
              <p> Create your first set above. </p>{" "}
            </div>
          ) : (
              ...
              {sets.map((set) => (
                ...
                >
                  ...
                  <div className="teacher-set-footer">
                    {" "}
                    <span>{set.flashcardCount || 0} words </span>{" "}
                    <span>
                      {" "}
                      {set.wordsToPlay === 0
                        ? "All words"
