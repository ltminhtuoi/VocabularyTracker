...
function StudentPerformance() {
  ...
  async function loadPerformance() {
    ...
    if (!response.ok) {
      throw new Error(data.message || "Failed to load student performance.");
    }
    setStudent(data);
  }
  async function loadWeakWords() {
    ...
    const data = await response.json();
    if (!response.ok) {
      throw new Error(data.message || "Failed to load weak words.");
    }
    setWeakWords(data);
  }
  async function loadAssignedSets() {
    ...
    const data = await response.json();
    if (!response.ok) {
      throw new Error(data.message || "Failed to load assigned sets.");
    }
  ...
  async function removeAssignment(assignmentId) {
    ...
    try {
      ...
      setAssignedSets((currentSets) =>
        currentSets.filter((assignment) => assignment.id !== assignmentId),
      );
    } catch (error) {
      setError(error.message);
    } finally {
      ...
      <div className="teacher-page">
        ...
        <main className="teacher-performance-content">
          ...
          <div className="teacher-performance-error">
            {" "}
            <div className="teacher-error-icon"> ! </div>{" "}
            <h2> Unable to load student </h2> <p> {error} </p>{" "}
            <button
      ...
      <main className="teacher-performance-content">
        ...
        <section className="teacher-student-profile">
          ...
          <div className="teacher-profile-info">
            ...
            <h1> {student.name} </h1> <p> @{student.username} </p>{" "}
          </div>{" "}
        </section>
        <section className="teacher-performance-stats">
          ...
          <div className="teacher-performance-stat">
            {" "}
            <span className="performance-stat-label"> Average score </span>{" "}
            <strong className={getScoreClass(averageScore)}>
              {" "}
          ...
          <div className="teacher-performance-stat">
            {" "}
            <span className="performance-stat-label"> Wrong words </span>{" "}
        ...
        <section className="teacher-performance-section">
          ...
          <div className="teacher-section-header">
            ...
            <div>
              {" "}
              <p className="teacher-eyebrow"> LEARNING MATERIAL </p>{" "}
              <h2> Assigned sets </h2>{" "}
            </div>{" "}
            <span>
              {" "}
              {assignedSets.length}{" "}
              {assignedSets.length === 1 ? "set" : "sets"}{" "}
            </span>{" "}
          </div>
          {assignedSets.length === 0 ? (
            <div className="teacher-empty">
              ...
              <p> This student doesn't have any flashcard sets yet. </p>{" "}
            </div>
          ) : (
            <div className="assigned-set-list">
              {" "}
              {assignedSets.map((assignment) => {
                const set =
                  assignment.flashcardSet || assignment.set || assignment;
                ...
                const flashcardCount =
                  set.flashcardCount ?? assignment.flashcardCount;
                const assignedAt = assignment.assignedAt;
                return (
                  <div key={assignmentId} className="assigned-set-card">
                    {" "}
                    <div className="assigned-set-icon"> ✦ </div>{" "}
                    <div className="assigned-set-info">
                      ...
                      <div className="assigned-set-meta">
                        ...
                        {flashcardCount !== undefined && (
                          <span> {flashcardCount} words </span>
                        )}
                        {assignedAt && (
        ...
        <section className="teacher-performance-section">
          <div className="teacher-section-header">
            ...
            <div>
              {" "}
              <p className="teacher-eyebrow"> PRACTICE ACTIVITY </p>{" "}
          ...
          {sessions.length === 0 ? (
            <div className="teacher-empty">
              ...
              <p>
                {" "}
                Practice sessions will appear here after the student starts
                learning.{" "}
              </p>{" "}
            </div>
          ) : (
              ...
              {sessions.map((session) => (
                ...
                >
                  <div className="teacher-session-main">
                    {" "}
                    <div className="teacher-session-icon"> ✓ </div>{" "}
                    <div>
                      {" "}
                      <h3> {session.setName} </h3>{" "}
                      <p> {formatDateTime(session.startedAt)} </p>{" "}
                    </div>{" "}
                  </div>{" "}
                  <div className="teacher-session-result">
                    {" "}
                    <strong className={getScoreClass(session.score)}>
                      {session.score}%
                    </strong>
                    <span>
                      {" "}
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      navigate(`/teacher/history/${session.id}`);
                    }}
                  >
                    {" "}
                    View answers{" "}
        ...
        <section className="teacher-performance-section">
          ...
          <div className="teacher-section-header">
            ...
            <div>
              {" "}
              <p className="teacher-eyebrow"> NEEDS PRACTICE </p>{" "}
              <h2> Weak words </h2>{" "}
            </div>{" "}
            <span>
              {" "}
              {weakWords.length}{" "}
              {weakWords.length === 1 ? "word" : "words"}{" "}
            </span>
          </div>
          {weakWords.length === 0 ? (
            <div className="teacher-strong-message">
              {" "}
              <div className="strong-message-icon"> ✓ </div>{" "}
              <div>
                <h3> No weak words yet</h3>
                <p>
                  {" "}
                  Great! There are no incorrect answers recorded for this
                  student.{" "}
                </p>
              </div>
            </div>
            ...
            <div className="weak-word-list">
              {" "}
              {weakWords.map((word) => (
                <div key={word.flashcardId} className="weak-word-card">
                  {" "}
                  <div className="weak-word-icon"> ! </div>{" "}
                  <div className="weak-word-content">
                    {" "}
                    <h3> {word.word} </h3> <p> {word.correctMeaning} </p>
                    <span>
                      {" "}
                      Last wrong: {formatDateTime(word.lastWrongAt)}{" "}
                    </span>{" "}
                  </div>{" "}
                  <div className="weak-word-count">
                    ...
                    <span>
                      {" "}
                      {word.wrongCount === 1 ? "mistake" : "mistakes"}{" "}
                    </span>{" "}
                  </div>{" "}
                </div>
              ))}{" "}
        ...
        <section className="teacher-performance-section">
          <div className="teacher-section-header">
            <div>
              <p className="teacher-eyebrow">ANSWER HISTORY</p>
          ...
          {attempts.length === 0 ? (
            <div className="teacher-empty">
              ...
              <p> Answer history will appear here after practice. </p>{" "}
            </div>
          ) : (
            <div className="teacher-attempt-list">
              {attempts.map((attempt) => (
                <div
                  ...
                  className={`teacher-attempt-card ${attempt.isCorrect ? "teacher-attempt-correct" : "teacher-attempt-wrong"}`}
                >
                  <div
                    className={`teacher-attempt-status ${attempt.isCorrect ? "status-correct" : "status-wrong"}`}
                  >
                    {attempt.isCorrect ? "✓" : "×"}{" "}
                  </div>{" "}
                  <div className="teacher-attempt-content">
                    ...
                    <div className="teacher-attempt-answer">
                      {" "}
                      <span> Selected: </span>
                      <strong> {attempt.selectedMeaning} </strong>
                    </div>
                    {!attempt.isCorrect && (
                      <div className="teacher-attempt-answer correct-answer">
                        {" "}
                        <span> Correct: </span>{" "}
                        <strong> {attempt.correctMeaning} </strong>{" "}
                      </div>
                    )}{" "}
                  </div>{" "}
                  <time> {formatDateTime(attempt.answeredAt)} </time>{" "}
                </div>
              ))}{" "}
            </div>
          )}
