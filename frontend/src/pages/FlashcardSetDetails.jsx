...
function FlashcardSetDetails() {
  ...
  async function loadSet() {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/FlashcardSet/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.message || "Failed to load flashcard set.");
      }
      setSet(data);
      setError("");
    } catch (error) {
  ...
  async function deleteFlashcard(flashcardId) {
    ...
    try {
      ...
      setError("");
      const response = await fetch(`${API_BASE_URL}/Flashcard/${flashcardId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await response.json();
      ...
      <div className="teacher-page">
        ...
        <main className="set-details-content">
          <div className="set-details-error">
            <div className="set-error-icon">!</div>
            <h2>Unable to load set</h2>
            <p>{error}</p>
            <button
      ...
      <main className="set-details-content">
        ...
        <section className="set-details-header">
          <div className="set-details-header-main">
            <div className="set-details-icon">✦</div>
            <div className="set-details-title">
              <p className="teacher-eyebrow">FLASHCARD SET</p>
              <h1>{set.name}</h1>
          ...
          <div className="set-details-header-count">
            ...
            <span>{flashcards.length === 1 ? "word" : "words"}</span>
          </div>
        </section>
        {error && (
        ...
        <section className="set-details-actions">
          <div>
            <p className="teacher-eyebrow">SET MANAGEMENT</p>
        ...
        <section className="set-details-section">
          <div className="teacher-section-header">
            <div>
              <p className="teacher-eyebrow">VOCABULARY</p>
              <h2>Flashcards</h2>
            </div>
            <span>
          ...
          {flashcards.length === 0 ? (
            <div className="teacher-empty">
              ...
              <p>
                This set is empty. Go back to thedashboard to import some
                vocabulary.
              </p>
              <button
            ...
            <div className="flashcard-list">
              {flashcards.map((flashcard, index) => (
                <article key={flashcard.id} className="flashcard-item">
                  ...
                  <div className="flashcard-content">
                    ...
                    <div className="flashcard-meaning">
                      ...
                      <strong>{flashcard.correctMeaning}</strong>
                    </div>
