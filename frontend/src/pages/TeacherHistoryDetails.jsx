...
function TeacherHistoryDetails() {
  ...
  async function loadDetails() {
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || "Failed to load practice session.");
      }
      setDetails(data);
    } catch (error) {
      ...
      <div className="teacher-page">
        ...
        <main className="teacher-history-details-content">
          ...
          <div className="teacher-history-error">
            ...
            <p> {error || "Practice session not found."} </p>{" "}
            <button
    ...
    <div className="teacher-page">
      ...
      <main className="teacher-history-details-content">
        ...
        <button
          ...
          onClick={() => navigate(`/teacher/students/${details.studentId}`)}
        >
          {" "}
        ...
        <section className="teacher-history-header">
          <div className="teacher-history-header-main">
            {" "}
            <div className="teacher-history-icon"> ✓ </div>{" "}
            <div>
              {" "}
              <p className="teacher-eyebrow"> PRACTICE SESSION </p>{" "}
              <h1> {details.setName} </h1> <p> {details.studentName} </p>{" "}
            </div>{" "}
          </div>{" "}
          <div className={`teacher-history-score ${getScoreClass(score)}`}>
            {" "}
            {score}%{" "}
          </div>{" "}
        </section>
        <section className="teacher-history-summary">
          ...
          <div className="teacher-history-summary-card">
            ...
            <strong>
              ...
              <small> / {details.totalQuestions} </small>{" "}
            </strong>
          </div>
          <div className="teacher-history-summary-card">
            {" "}
        ...
        <section className="teacher-history-answers">
          ...
          <div className="teacher-section-header">
            ...
            <div>
              {" "}
              <p className="teacher-eyebrow"> ANSWER BREAKDOWN </p>
              <h2> Answers </h2>{" "}
            </div>{" "}
            <span>
              {" "}
              {attempts.length}{" "}
              {attempts.length === 1 ? "question" : "questions"}{" "}
            </span>{" "}
          </div>
          {attempts.length === 0 ? (
            <div className="teacher-empty">
              ...
              <p> There are no answers for this practice session. </p>
            </div>
          ) : (
            <div className="teacher-history-answer-list">
              ...
              {attempts.map((attempt, index) => (
                <div
                  ...
                  className={`teacher-history-answer-card ${attempt.isCorrect ? "history-answer-correct" : "history-answer-wrong"}`}
                >
                  ...
                  <div
                    className={`teacher-history-answer-status ${attempt.isCorrect ? "status-correct" : "status-wrong"}`}
                  >
                    {" "}
                    {attempt.isCorrect ? "✓" : "×"}{" "}
                  </div>
                  <div className="teacher-history-answer-content">
                    ...
                    <div className="teacher-history-word-row">
                      ...
                      <h3> {attempt.word}</h3>{" "}
                      <span
                        className={`teacher-history-status-label ${attempt.isCorrect ? "status-correct" : "status-wrong"}`}
                      >
                        {" "}
                    ...
                    <p className="teacher-history-answer-time">
                      {" "}
                      Answered {formatDateTime(attempt.answeredAt)}{" "}
                    </p>{" "}
                  </div>{" "}
                </div>
              ))}{" "}
